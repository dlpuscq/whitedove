...
(function($) {
	...
	require.prototype = {
		 ...
		 * @update:	手机号码更改发送验证码按钮在倒计时没有为0时不出现
		 */
...
$(function() {
	...
	$("#btn_part1").click(function() {
		...
		$.ajax({
			type : "POST",
			url : baselocation + '/createuser',
			data : params,
			dataType : "json",
			success : function(result) {
				if (result.status == 200) {
                    $(".part4").children("h3").text("恭喜，您已注册成功，现在开始您的学习之旅吧！");
                    $(".part1").hide();
                    $(".part4").show();
...
$(function() {
    ...
    $("#verifyYz").click(function() {
        $.ajax({
            type : "POST",
            url : baselocation + '/sendEmailRegister',
            data : {
                email : backemail,
            },
            dataType : "json",
            success : function(result) {
                if (result.status == 200) {
                    document.getElementById("checkchange").value = result.data;
                }
            }
        })
    });
	...
	$("#btn_Pswpart2").click(function() {
		...
		var email = $("input[name='email']").val();
		var emailSign = document.getElementById("checkchange").value;
		if (!verifyCheck._click()) return;
		$.ajax({
			type : "POST",
	...
	$("#btn_Pswpart3").click(function() {
		...
		$.ajax({
			...
			success : function(result) {
				if (result.status == 200) {
					$(".part4").children("h3").text("恭喜" + result.data + "您已成功找回密码,现在开始您的学习之旅吧!");
					$(".part3").hide();
					$(".part4").show();
					$(".step li").eq(2).addClass("on");
					countdown({
